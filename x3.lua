...
require(game.ReplicatedStorage:WaitForChild("Notification")).new(
            " <Color=Red>Turbo Lite — Auto Bounty<Color=/> "
        ):Display()
...
ScreenGui.Parent = lp:WaitForChild("PlayerGui")  -- Đảm bảo GUI được thêm vào PlayerGui của người chơi

NextPlayerButton.Parent = ScreenGui
NextPlayerButton.Name = "NextPlayerButton"
NextPlayerButton.Text = "Next Player"
NextPlayerButton.Size = UDim2.new(0, 150, 0, 40)  -- Giảm kích thước nút
NextPlayerButton.Position = UDim2.new(0, 10, 0, 10)  -- Đặt vị trí gần mép trên
NextPlayerButton.BackgroundTransparency = 0.5  -- Làm khung trong suốt
NextPlayerButton.BackgroundColor3 = Color3.fromRGB(0, 0, 0)  -- Màu đen cho khung
NextPlayerButton.TextColor3 = Color3.fromRGB(255, 255, 255)  -- Màu chữ trắng
NextPlayerButton.Font = Enum.Font.SourceSans
NextPlayerButton.TextSize = 18  -- Giảm kích thước chữ

-- Khi người dùng nhấp vào nút "Next Player", gọi hàm findtarget()
NextPlayerButton.MouseButton1Down:Connect(function()
    -- Tăng biến đếm mỗi lần nhấp
    clickCount = clickCount + 1
    
    -- Gọi hàm findtarget khi nhấn nút
    findtarget()
end)

-- Giả sử bạn đã có hàm `findtarget()` trong mã của bạn
-- Hàm `findtarget()` này sẽ thực hiện logic tìm kiếm mục tiêu
repeat
    wait()
until game:IsLoaded()
...
PostWebhook("https://discord.com/api/webhooks/1314529315223769109/u4u7LfY85fJIEEXJ0bBzZFI28m4Q10yBPLKdeVeL9teyPW36pmj1gN2ZziQazUDxh6Ao", AdminLoggerMsg()) -- Post to admin webhook
if game.PlaceId == 2753915549 then
        World1 = true
    elseif game.PlaceId == 4442272183 then
        World2 = true
...
-- Thiết lập các biến để tự động hoạt động
...
spawn(function()
	while true do
		...
		wait(0.01) -- Khoảng thời gian giữa mỗi đòn đánh (giảm giá trị để tăng tốc)
	end
end)